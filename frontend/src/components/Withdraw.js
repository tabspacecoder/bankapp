...
const handleOnSubmit=(e)=>{
  e.preventDefault();
  axios.post(`http://localhost:8080/api/account/withdraw-amount`,{
  customerId:window.localStorage.getItem("customerId"),
  password:window.localStorage.getItem("password"),
  amount:amount,
  accountId:selectedAccId
}).then(res=>{
  enqueueSnackbar("Successfully withdrawed!",{variant:"success"})
  setAmount(0);
    ...
    <>
    {
      ...
      <form className="Auth-form">
        <div className="Auth-form-content">
          <h3 className="Auth-form-title">Withdraw</h3>
          <Select
